...
public class LevelLoad : MonoBehaviour
{
    ...
    {
        if (SpecialCardArea != null && cardUserPref.specialCards.Count > 0) // error handling
        {
            ...
            if (specialCard != null) // error handling
            {
                ...
                // remove from the avaliable special cards list in CardUserPref script
                cardUserPref.specialCards.RemoveAt(specialInt);

                // add the special card to the users deck
        ...
        {
            Debug.LogError("SpecialCardArea is not assigned or specialCards list is empty!"); // debugging for out of range exceptions
        }
    }
